...
app.post('/saveRecipe/:email', async (req, res) => {
    const client = await MongoClient.connect(dbUrl)
    try {
        if (req.params.email) {
            const db = await client.db('recipes')
            let user = await db.collection('users').findOne({ email: req.params.email })
            if (user) {
                await db.collection('saved recipes').insertOne({recipeName:req.body, userId:user._id, userEmail:user.email})
                res.status(201).send({ message: 'Recipe successfully saved', data: req.body })
            }
            else {
                res.status(400).send({ message: `User with email Id ${req.params.email} does not exist` })
            }
        }
        else {
